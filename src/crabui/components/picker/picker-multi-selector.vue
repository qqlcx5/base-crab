 ...
 * @LastEditors: sanshui
 * @LastEditTime: 2022-06-02 16:31:04
...
<template>
    ...
    >
        <picker-view-column
            v-for="(rangeItem, rangeIndex) in range"
            :key="rangeIndex"
        >
            <view
...
export default defineComponent({
    ...
    props: {
        ...
        range: {
            ...
            validator: function (value) {
                if (value.length === 0 || (['Array'].includes(getVarType(value[0])) && ['Object', 'String'].includes(getVarType(value[0][0])))) {
                    return true
                }
                throw (new Error('range值只能是Array<Array<String>> / Array<Array<Object>>'))
            }
        },
        rangeKey: {
            type: String,
    ...
    setup(props) {
        ...
        function formatValue(indexs) {
            currentDate.value = indexs || currentDate.value || [0, 0]
            pickerInfo.value = {
                chosen: props.range.map((o, i) => {
                    let ret = o[currentDate.value[i]]
                    if (props.rangeKey) {
                        ret = ret[[props.rangeKey]]
                    }
                    return getVarType(ret) === 'Object' ? JSON.stringify(ret) : ret
                }),
